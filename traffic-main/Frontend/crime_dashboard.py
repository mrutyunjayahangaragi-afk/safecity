...
def make_dashboard(df: pd.DataFrame, out_path: str):
    ...
    gs = GridSpec(4, 4, figure=fig, hspace=0.45, wspace=0.38,
    ...
    if hazards:
        ...
        htype_counts = df_hazards["hazard_type"].value_counts()
        ax6.bar(htype_counts.index, htype_counts.values, color=ACCENT, alpha=0.9)
        ax6.set_title("Active Road Hazards by Type", fontweight="bold", pad=10)
        ax6.set_xlabel("Hazard Type")
        ax6.set_ylabel("Count")
        ax6.grid(axis="y")
        
        # Hazard Severity vs Delay
        ax7 = fig.add_subplot(gs[3, 2:])
        if "expected_delay" in df_hazards.columns:
            sc = ax7.scatter(df_hazards["severity"], df_hazards["expected_delay"], 
                        c=df_hazards["safety_impact"], cmap="Reds", s=100, alpha=0.8, edgecolors=DARK_BG)
            ax7.set_title("Hazard Severity vs Expected Delay (Mins)", fontweight="bold", pad=10)
            ax7.set_xlabel("Severity (1-10)")
            ax7.set_ylabel("Expected Delay")
            ax7.grid(True)
            cbar = plt.colorbar(sc, ax=ax7)
            cbar.set_label("Safety Impact")
